...
class PainterLongVideo:
    ...
    def execute(
        ...
        print(f"[PainterLongVideo] has_prev={has_prev}, has_start={has_start}, has_end={has_end}")
        if has_prev:
            print(f"[PainterLongVideo] previous_video.shape={previous_video.shape}")
        ...
        # (a2) 从 previous_video 均匀采样多帧，增强身份一致性
        # 当前视频末帧可能无人脸（如衣服特写），采样更多帧可覆盖有人脸的帧
        if has_prev and previous_video.shape[0] > 1:
            n_samples = min(4, previous_video.shape[0] - 1)
            # 均匀采样（不包含最后一帧，已在上面添加）
            indices = torch.linspace(0, previous_video.shape[0] - 2, n_samples).long()
            print(f"[PainterLongVideo] Multi-frame sampling: n_samples={n_samples}, indices={indices.tolist()}, from {previous_video.shape[0]} frames")
            for idx in indices:
                frame = previous_video[idx:idx+1]
                frame_resized = comfy.utils.common_upscale(
                    frame.movedim(-1, 1), width, height, "bilinear", "center"
                ).movedim(1, -1)
                ref_latents.append(vae.encode(frame_resized[:, :, :, :3]))
        ...
        print(f"[PainterLongVideo] Total ref_latents: {len(ref_latents)} (has_initial_ref={initial_reference_image is not None})")
