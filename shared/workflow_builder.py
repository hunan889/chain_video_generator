...
from shared.enums import ModelType, GenerateMode
from shared.schemas import LoraInput

logger = logging.getLogger(__name__)

# ---------------------------------------------------------------------------
# Module-level path variables — set defaults, override via configure()
# ---------------------------------------------------------------------------
WORKFLOWS_DIR: Path = Path("workflows")
COMFYUI_PATH: Path = Path("ComfyUI")
LORAS_PATH: Path = Path("config/loras.yaml")
LORAS_DIR: Path = COMFYUI_PATH / "models" / "loras"


def configure(
    workflows_dir: "Path | str | None" = None,
    comfyui_path: "Path | str | None" = None,
    loras_path: "Path | str | None" = None,
) -> None:
    ...
    global DIFFUSION_DIR, TEXT_ENCODERS_DIR
    ...
    if loras_path is not None:
        LORAS_PATH = Path(loras_path)
    # Rebuild caches that depend on paths
    _lora_name_map.clear()
    _lora_name_map.update(_load_lora_name_map())
...
def build_story_workflow(
    ...
    if loras:
        normalized_for_tw = [l if hasattr(l, "name") else LoraInput(**l) for l in loras]
    ...
    if loras:
        normalized = [l if hasattr(l, "name") else LoraInput(**l) for l in loras]
...
def build_merged_story_workflow(
    ...
    for seg_idx, seg in enumerate(segments):
        ...
        seg_loras = [l if hasattr(l, "name") else LoraInput(**(l if isinstance(l, dict) else {"name": l})) for l in seg_loras_raw]
